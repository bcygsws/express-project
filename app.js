 ...
 * b.间隙锁（gap locks）
 * 间隙锁-gap locks
 * b1.间隙锁是InnoDB在可重复读（RR）隔离级别为解决幻读问而引入的机制
 * 
 * 
 * c.临键锁（next-key）
 * d.意向锁
 ...
 * 
 * 乐观锁:
 * 认为别的线程不会修改数据，所以不会上锁；但在更新的时候会判断 在次期间，有没有其他线程更新这个数据
 * 适用于读多写少的场景，也是减少锁竞争的冲突，提高系统的吞吐量
 * 
 * 悲观锁：
 * 认为别的线程会修改数据，所以每次在拿数据时都会上锁，其他线程修改数据的操作会被阻塞直到自己拿到锁
 * 适用于写多读少的场景；因为，此时如果使用乐观锁的话，就会出现大量的操作冲突，会导致应用层不断地retry,从而
 * 降低系统性能
 * 
 * 
 *
